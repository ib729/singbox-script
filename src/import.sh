...
in_conf() {
    ...
    [[ $? != 0 ]] && warn "Unable to read file: $1" && return
    ...
    if [[ $host ]]; then
        ...
        vmess | vless)
            [[ $net_type == "http" ]] && {
                net=http
                is_tips_msg="New configuration name: (VMess-HTTP-$port.json)"
            }
        ...
        dokodemo-door)
            add door
            is_tips_msg="New configuration name: (Direct-$port.json)"
            ;;
        *socks*)
            add $is_protocol
            ;;
        *)
            is_not_in_conf=1
            msg "Import not supported for $1"
            ;;

        esac
    fi
    [[ ! $is_not_in_conf ]] && msg "Import: $1 $is_tips_msg" && rm $1
}
is_change=1
is_dont_auto_exit=1
is_dont_test_host=1
...
if [[ ${is_list[@]} ]]; then
    ...
    err "No configurations found to import"
fi