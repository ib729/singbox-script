...
get_port() {
    ...
    while :; do
        ...
        if [[ $is_count -ge 233 ]]; then
            err "Failed to get available port after 233 attempts. Check port usage"
        fi
...
is_port_used() {
    ...
    is_cant_test_port=1
    msg "$is_warn Unable to check port availability"
    msg "Execute: $(_yellow "${cmd} update -y; ${cmd} install net-tools -y") to fix this"
}
...
ask() {
    ...
    msg $is_opt_msg
    [[ ! $is_opt_input_msg ]] && is_opt_input_msg="Select [\e[91m1-${#is_tmp_list[@]}\e[0m]:"
    ...
    while :; do
        ...
        if [[ ! $is_tmp_list ]]; then
            [[ $(grep port <<<$is_ask_set) ]] && {
                [[ ! $(is_test port "$REPLY") ]] && {
                    msg "$is_err Enter valid port (1-65535)"
                    continue
                }
                if [[ $(is_test port_used $REPLY) && $is_ask_set != 'door_port' ]]; then
                    msg "$is_err Cannot use port ($REPLY)"
                    continue
                fi
            }
            [[ $(grep path <<<$is_ask_set) && ! $(is_test path "$REPLY") ]] && {
                [[ ! $tmp_uuid ]] && get_uuid
                msg "$is_err Enter valid path, e.g., /$tmp_uuid"
                continue
            }
            [[ $(grep uuid <<<$is_ask_set) && ! $(is_test uuid "$REPLY") ]] && {
                [[ ! $tmp_uuid ]] && get_uuid
                msg "$is_err Enter valid UUID, e.g., $tmp_uuid"
                continue
            }
            [[ $(grep ^y$ <<<$is_ask_set) ]] && {
...
create() {
    ...
    client)
        ...
        is_client=1
        get info $2
        [[ ! $is_client_id_json ]] && err "($is_config_name) does not support client configuration generation"
...
change() {
    ...
    if [[ $2 ]]; then
        ...
        *)
            ...
            err "Cannot recognize ($2) change type"
            ;;
        esac
    fi
    [[ $is_try_change ]] && return
    ...
    1)
        # new port
        is_new_port=$3
        [[ $host && ! $is_caddy || $is_no_auto_tls ]] && err "($is_config_file) does not support port change"
        if [[ $is_new_port && ! $is_auto ]]; then
            [[ ! $(is_test port $is_new_port) ]] && err "Enter valid port (1-65535)"
            [[ $is_new_port != 443 && $(is_test port_used $is_new_port) ]] && err "Cannot use port ($is_new_port)"
        fi
        [[ $is_auto ]] && get_port && is_new_port=$tmp_port
        [[ ! $is_new_port ]] && ask string is_new_port "Enter new port:"
    ...
    2)
        # new host
        is_new_host=$3
        [[ ! $host ]] && err "($is_config_file) does not support domain change"
        [[ ! $is_new_host ]] && ask string is_new_host "Enter new domain:"
    ...
    3)
        # new path
        is_new_path=$3
        [[ ! $path ]] && err "($is_config_file) does not support path change"
        [[ $is_auto ]] && get_uuid && is_new_path=/$tmp_uuid
        [[ ! $is_new_path ]] && ask string is_new_path "Enter new path:"
    ...
    4)
        ...
        if [[ $ss_password || $password ]]; then
            ...
            err "($is_config_file) does not support password change"
        fi
        [[ ! $is_new_pass ]] && ask string is_new_pass "Enter new password:"
    ...
    5)
        # new uuid
        is_new_uuid=$3
        [[ ! $uuid ]] && err "($is_config_file) does not support UUID change"
        [[ $is_auto ]] && get_uuid && is_new_uuid=$tmp_uuid
        [[ ! $is_new_uuid ]] && ask string is_new_uuid "Enter new UUID:"
        add $net auto $is_new_uuid
        ;;
    6)
        # new method
        is_new_method=$3
        [[ $net != 'ss' ]] && err "($is_config_file) does not support encryption method change"
    ...
    7)
        # new remote addr
        is_new_door_addr=$3
        [[ $net != 'direct' ]] && err "($is_config_file) does not support remote address change"
        [[ ! $is_new_door_addr ]] && ask string is_new_door_addr "Enter new remote address:"
    ...
    8)
        # new remote port
        is_new_door_port=$3
        [[ $net != 'direct' ]] && err "($is_config_file) does not support remote port change"
        [[ ! $is_new_door_port ]] && {
            ask string door_port "Enter new remote port:"
    ...
    9)
        ...
        is_new_public_key=$4
        [[ ! $is_reality ]] && err "($is_config_file) does not support key change"
        if [[ $is_auto ]]; then
            get_pbk
            add $net
        else
            [[ $is_new_private_key && ! $is_new_public_key ]] && {
                err "Cannot find Public key"
            }
            [[ ! $is_new_private_key ]] && ask string is_new_private_key "Enter new Private key:"
            [[ ! $is_new_public_key ]] && ask string is_new_public_key "Enter new Public key:"
    ...
    10)
        ...
        [[ ! $is_new_servername ]] && ask string is_new_servername "Enter new serverName:"
    ...
    11)
        ...
        is_new_proxy_site=$3
        [[ ! $is_caddy && ! $host ]] && {
            err "($is_config_file) does not support changing the proxy site."
        }
        [[ ! -f $is_caddy_conf/${host}.conf.add ]] && err "Unable to configure proxy site."
        [[ ! $is_new_proxy_site ]] && ask string is_new_proxy_site "Enter new proxy site (e.g. example.com):"
        ...
        msg "\nProxy site updated to: $(_green $proxy_site) \n"
        ;;
    12)
        # new socks user
        [[ ! $is_socks_user ]] && err "($is_config_file) does not support changing username (Username)."
        ask string is_socks_user "Enter new username (Username):"
        add $net
        ;;
    esac
}
...
del() {
    ...
    if [[ $is_config_file ]]; then
        if [[ $is_main_start && ! $is_no_del_msg ]]; then
            msg "\nDelete config file?: $is_config_file"
            pause
        fi
    ...
    if [[ ! $(ls $is_conf_dir | grep .json) && ! $is_change ]]; then
        warn "Config directory is empty! You just deleted the last config file."
        is_conf_dir_empty=1
    fi
...
uninstall() {
    if [[ $is_caddy ]]; then
        ...
        ask string y "Uninstall ${is_core_name}? [y]:"
    fi
    ...
    _green "\nUninstall completed!"
    msg "What should be improved in the script? Please share feedback."
    ...
    [[ $is_test_run && ! $is_new_install ]] && {
        ...
        if [[ ! $(pgrep -f $is_run_bin) ]]; then
            ...
            [[ ! $is_no_manage_msg ]] && {
                ...
                _yellow "Detected failure, running test automatically."
                get test-run
                _yellow "Test finished, press Enter to exit."
            }
        fi
    }
}
...
add() {
    ...
    if [[ $is_lower ]]; then
        ...
        *)
            ...
            [[ ! $is_new_protocol ]] && err "Unrecognized ($1), use: $is_core add [protocol] [args... | auto]"
            ;;
        esac
    fi
    ...
    *reality*)
    ...
    [[ $1 && ! $is_change ]] && {
        msg "\nProtocol: $is_new_protocol"
        # err msg tips
        is_err_tips="\n\nUse: $(_green $is_core add $1 $is_add_opts) to add $is_new_protocol config"
    }
    ...
    if [[ $is_no_auto_tls && ! $is_use_tls ]]; then
        err "$is_new_protocol does not support manual TLS configuration."
    fi
    ...
    if [[ $2 ]]; then
        ...
        if [[ $is_use_port ]]; then
            ...
            [[ $(is_test port_used $is_use_port) && ! $is_gen ]] && {
                err "Cannot use port ($is_use_port). $is_err_tips"
            }
            port=$is_use_port
        fi
    ...
    if [[ $is_use_tls ]]; then
        ...
        # set host
        [[ ! $host ]] && ask string host "Enter domain:"
        # test host dns
        get host-test
    else
        ...
        if [[ $is_main_start ]]; then
            ...
            # set port
            [[ ! $port ]] && ask string port "Enter port:"

            case ${is_new_protocol,,} in
            socks)
                # set user
                [[ ! $is_socks_user ]] && ask string is_socks_user "Set username:"
                # set password
                [[ ! $is_socks_pass ]] && ask string is_socks_pass "Set password:"
                ;;
            shadowsocks)
                # set method
                [[ ! $ss_method ]] && ask set_ss_method
                # set password
                [[ ! $ss_password ]] && ask string ss_password "Set password:"
                ;;
            esac
    ...
    if [[ $is_new_protocol == 'Direct' ]]; then
        # set remote addr
        [[ ! $door_addr ]] && ask string door_addr "Enter target address:"
        # set remote port
        [[ ! $door_port ]] && ask string door_port "Enter target port:"
    fi
    ...
    if [[ $(grep 2022 <<<$ss_method) ]]; then
        ...
        [[ $ss_password ]] && {
            ...
            if [[ $? != 0 ]]; then
                warn "Shadowsocks protocol ($ss_method) does not support password ($(_red_bg $ss_password))\n\nYou can use: $(_green $is_core ss2022) to generate a supported password.\n\nThe script will auto-create a usable password :)"
                ss_password=
                # create new json.
                json_str=
            fi
...
get() {
    ...
    file)
        ...
        [[ ! $is_all_json ]] && err "Cannot find related config file: $2"
    ...
    protocol)
        ...
        socks*)
            ...
            ;;
        *)
            err "Unrecognized protocol: $is_config_file"
            ;;
        esac
    ...
    host-test) # test host dns record; for auto *tls required.
        ...
        if [[ ! $(grep $ip <<<$is_host_dns) ]]; then
            msg "\nPlease point ($(_red_bg $host)) to ($(_red_bg $ip))"
            msg "\nIf using Cloudflare, in DNS disable Proxy status (DNS only)"
            ask string y "I have confirmed DNS [y]:"
            get ping
            if [[ ! $(grep $ip <<<$is_host_dns) ]]; then
                _cyan "\nTest result: $is_host_dns"
                err "Domain ($host) does not resolve to ($ip)"
            fi
        fi
        ;;
    ssss | ss2022)
    ...
    test-run)
        ...
        [[ $? != 0 ]] && {
            _yellow "\nCannot run test, please check systemctl status.\n"
            return
        }
        is_no_manage_msg=1
        if [[ ! $(pgrep -f $is_core_bin) ]]; then
            _yellow "\nTest running $is_core_name ..\n"
            manage start &>/dev/null
            if [[ $is_run_fail == $is_core ]]; then
                _red "$is_core_name failure info:"
                $is_core_bin run -c $is_config_json -C $is_conf_dir
            else
                _green "\nTest passed, started $is_core_name ..\n"
            fi
        else
            _green "\n$is_core_name is running, skip test\n"
        fi
        if [[ $is_caddy ]]; then
...
info() {
    ...
    if [[ $is_url ]]; then
        ...
        [[ $is_insecure ]] && {
            warn "Some clients (e.g., V2rayN) require setting allowInsecure to true, or enable insecure connections"
        }
    fi
...
update() {
    ...
    2 | sh)
        ...
        ;;
    3 | caddy)
        [[ ! $is_caddy ]] && err "Updating Caddy is not supported."
    ...
    *)
    ...
    if [[ $is_new_ver ]]; then
        ...
        [[ $is_run_ver == $latest_ver ]] && {
            msg "\n$is_show_name is already the latest version.\n"
            exit
        }
        msg "\nNew $is_show_name version found: $(_green $latest_ver)\n"
        is_new_ver=$latest_ver
    fi
    download $is_update_name $is_new_ver
    msg "Update successful, current $is_show_name version: $(_green $is_new_ver)\n"
    msg "$(_green Please see release notes: https://github.com/$is_update_repo/releases/tag/$is_new_ver)\n"
...
is_main_menu() {
    ...
    5)
        ...
        manage $REPLY &
        msg "\nManage action: $(_green $is_do_manage)\n"
        ;;
    6)
    ...
    8)
        ...
        ;;
    9)
        ask list is_do_other "Enable BBR View Logs Test Run Reinstall Script Set DNS"
        case $REPLY in
        1)
            load bbr.sh
            _try_enable_bbr
...
main() {
    ...
    dd | ddel | fix | fix-all)
        ...
        *)
            is_dont_auto_exit=1
            [[ ! $2 ]] && {
                err "Cannot find parameters to delete"
            } || {
                for v in ${@:2}; do
                    del $v
                done
    ...
    debug)
        is_debug=1
        get info $2
        warn "If you need to copy, redact *uuid, *password, *host, *key to avoid leaks."
        ;;
    fix-config.json)
        create config.json
        ;;
    ...
    start | stop | r | restart)
        [[ $2 && $2 != 'caddy' ]] && err "Unrecognized ($2), use: $is_core $1 [caddy]"
        manage $1 $2 &
        ;;
    t | test)
        get test-run
